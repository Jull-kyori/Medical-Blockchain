...
function createGenesisBlock() {
  const data = {
    patientId: 'GENESIS', age: 0, sex: '-',
    diagnosis: 'System Initialized', doctor: 'System',
...
function addBlock(patientData) {
  ...
  const newBlock = { index, timestamp, data: patientData, previousHash: previousBlock.hash, hash, nonce };
  blockchain.push(newBlock);
  return newBlock;
}

function addMultipleBlocks(records) {
  return records.map(record => addBlock(record));
}
